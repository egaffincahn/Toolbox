...
myaddress = input('Enter email address: ','s');
% mypassword = inputdlg('Enter password');
...
setpref('Internet','SMTP_Password',logindlg('Password','only'));
...
try
    sendmail(myaddress, input('Enter subject: ','s'), input('Enter text body: ','s'));
catch me
    fprintf(['If SetupEmail fails due to authentication, one problem may be\n', ...
        'Google preventing 3rd party apps from sending emails. See ',...
        '<a href = "matlab:web(''%s'', ''-browser'')">%s</a>.\n'], ...
        'https://www.google.com/settings/security/lesssecureapps', 'Google help')   
    rethrow(me)
end
